...
df_temp = woe_discrete(df_inputs_prepr,'grade',df_targets_prep)
# plot_by_woe(df_temp)
...
df_temp = woe_discrete(df_inputs_prepr,'home_ownership',df_targets_prep)
...
df_temp = woe_discrete(df_inputs_prepr,'addr_state',df_targets_prep)
...
df_temp = woe_discrete(df_inputs_prepr, 'verification_status', df_targets_prep)
# plot_by_woe(df4_verification_status)
lst_IV = append_IV_list(df_temp, lst_IV) # append variable and IV values to list IV

# df5_purpose : combine
df_temp = woe_discrete(df_inputs_prepr, 'purpose', df_targets_prep)
...
df_temp = woe_discrete(df_inputs_prepr, 'initial_list_status', df_targets_prep)
...
# term_int
df_temp = woe_ordered_continuous(df_inputs_prepr, 'term_int', df_targets_prep)
# plot_by_woe(df_temp) ---------------------------
lst_IV = append_IV_list(df_temp, lst_IV) # append variable and IV values to list IV
...
# emp_length_int ---------------------------
df_temp = woe_ordered_continuous(df_inputs_prepr, 'emp_length_int', df_targets_prep)
# plot_by_woe(df_temp)
lst_IV = append_IV_list(df_temp, lst_IV) # append variable and IV values to list IV
...
dataframe_name = 'df_inputs_prepr'
file_name = 'temp/df_inputs_prepr.csv'

# Try Except: https://www.w3schools.com/python/python_try_except.asp
try:
    # start from column HA = good_bad
    df_inputs_prepr[:51].to_csv(file_name)
except Exception as e:
    print('exec loan_data.to_csv(file_name); error = ' + e)
else:
    print(f'DataFrame {dataframe_name} is written to CSV File = {file_name} successfully.')
...
#***************************************************************************#
'''
df_inputs_prepr.iloc[:51,(0:2, 206:)].to_csv(file_name)
df_inputs_prepr.iloc[:51, 205:]

df_tmp = pd.concat([df_inputs_prepr.iloc[:51,1:4], df_inputs_prepr.iloc[:52,206:]], axis=1)


# -------------
df_inputs_prepr['addr_state:]


# df[df['A'].str.contains("hello")
df_inputs_prepr[df_inputs_prepr.columns.str.contains('addr_state')]

df.loc[:,df.columns.str.startswith('al')]  # col name start with

df_tmp = df_inputs_prepr.loc[:10, df_inputs_prepr.columns.str.startswith('addr_st')]
